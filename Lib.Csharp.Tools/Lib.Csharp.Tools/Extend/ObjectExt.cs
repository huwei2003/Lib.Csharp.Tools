    ...
    public static class ObjectExt
    {
        ...
        internal static object JsonToArray(this string s, Type type)
        {
            ...
            try
            {
                ...
                var arr = s.ToObject<List<Dictionary<string, object>>>();
                if (arr.Count > 0)
                {
        ...
        internal static object ToJsonable(this object obj)
        {
            ...
            switch (typeName)
            {
                default:
                    {
                        if (!type.IsValueType)
                        {
                            if (type.IsArray)
                            {
                                ...
                                return eobjs.Select(a => a.ToJsonable()).ToArray();
                            }
                            if (type.Namespace.In("System.Linq", "System.Collections.Generic"))
                            {
                                var eobjs = obj as IEnumerable<object>;
                                if (eobjs == null)
                                {
                ...
                case "DataRow":
                    {
                        return DataRowExt.ToDictionary(((DataRow)obj));
                    }
                case "DataSet":
                    {
                ...
                case "DbParameter[]":
                    return ((DbParameter[])obj).Select(a => a.Value);
            }
            return obj;
        }
